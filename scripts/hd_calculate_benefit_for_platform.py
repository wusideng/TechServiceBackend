...
withdraw_fee = 5
# 技师提成百分比
tech_benefit_rate = 0.5
tech_benefit_rate_add = 0.7
# 渠道费（平台维护费）
maintenance_fee = 10
...
def main(start_date, end_date, actual_travel_fees: List[dict] = None):
    ...
    with Session(engine) as session:
        ...
        for order in orders:
            ...
            print("\n")
            print(
                f"技师收益：({benefit_info['order_cost']}-{benefit_info['travel_fee']})*{benefit_info['tech_benefit_rate']}-{benefit_info['maintenance_fee']}-{benefit_info['withdraw_fee']}+{benefit_info['actual_travel_fee']} = {benefit_info['tech_benefit']}"
            )
            print(
...
def get_benefit_info(
    ...
    order_id,
    create_order_time,
    tech_benefit_current_rate,
    maintenance_current_fee
):
    order_cost = float(order_cost)
    tax_fee = order_cost * tax_rate
    tech_benefit = (
        (order_cost - travel_fee) * tech_benefit_current_rate
        - withdraw_fee_for_current_order
        - maintenance_current_fee
        + actual_travel_fee
    )
    ...
    benefit_info = {
        ...
        "maintenance_fee":maintenance_current_fee,
        "tech_benefit_rate":tech_benefit_current_rate
...
def calculate_benefit(order: T_Order, session: Session):
    ...
    if order.parent_order_id:
        travel_fee = 0
        actual_travel_fee = 0
        tech_benefit_current_rate = tech_benefit_rate_add
        maintenance_current_fee = maintenance_fee_add
    else:
