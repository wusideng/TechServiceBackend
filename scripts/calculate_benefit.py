...
def main(date_str_input: str = None, actual_travel_fees: List[dict] = None):
    ...
    with Session(engine) as session:
        statement = (
            ...
            .where(
                ...
                cast(T_Order.create_order_time, Date) == date_str,
            )
        )
        ...
        for order in orders:
            ...
            print(f"渠道费：{maintainence_fee}")
            print(f"手续费：{benefit_info['withdraw_fee']}")
            print("\n")
            print(
                f"技师收益：({benefit_info['order_cost']}-{benefit_info['travel_fee']})*{tech_benefit_rate}-{maintainence_fee}-{benefit_info['withdraw_fee']}+{benefit_info['actual_travel_fee']} = {benefit_info['tech_benefit']}"
            )
            print(
