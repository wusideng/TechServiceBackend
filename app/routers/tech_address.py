...
async def get_tech_addresses(openid: str, session: Session = Depends(get_session)):
    """获取用户地址列表"""
    try:
        statement = (
            select(TechAddress)
            .where(TechAddress.openid == openid)
            .order_by(desc(TechAddress.create_time))
        )
...
async def add_tech_address(
    address: TechAddress, session: Session = Depends(get_session)
):
    """添加新地址"""
    try:
        # 创建新地址
        # 如果设置为默认地址，先将其他地址设为非默认
        statement = select(TechAddress).where(
            TechAddress.openid == address.openid,
            TechAddress.is_default == True,
        )
        default_addresses = session.exec(statement).all()
        len_default_addresses = len(default_addresses)
        if address.is_default and address.openid:
            for addr in default_addresses:
                addr.is_default = False
                session.add(addr)
        if len_default_addresses == 0 or not default_addresses:
...
async def update_tch_address(
    address_id: int,
    address_update: TechAddress,
    session: Session = Depends(get_session),
):
    """更新地址"""
    try:
        # 获取要更新的地址
        statement = select(TechAddress).where(TechAddress.id == address_id)
        ...
        if address_update.is_default and address.openid:
            default_statement = select(TechAddress).where(
                TechAddress.openid == address.openid,
                TechAddress.is_default == True,
                TechAddress.id != address_id,
            )
...
async def get_tech_default_address(
