...
def calculate_extends_for_tech(openid, session: Session):
    ...
    print('order_actual_sum', order_actual_sum)
    if order_sum != 0:
        # 加钟总金额
        extend_orders_actual_sum = sum(
                (orders_actual.actual_fee_received / 100 if orders_actual.actual_fee_received is not None else 0) - 
                2 * orders_actual.travel_cost 
                for orders_actual in extend_orders_actual
            )
        print('extend_orders_actual_sum', extend_orders_actual_sum)
